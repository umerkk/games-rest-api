    ...
    public static async init(): Promise<void> {
        try {
            const defaultConnection = await Mongoose.connect(
                process.env.NODE_ENV === 'production' ? process.env.DB_URL as string : 'mongodb+srv://demoUser:<ASK PASSWORD>@metacasetinterview.udrwq.mongodb.net/metacast?retryWrites=true&w=majority',
                {
